...
import anthropic
from access_keys import key_openai, key_claude
import os
from openai import OpenAI

os.environ["OPENAI_API_KEY"] = key_openai
os.environ["ANTHROPIC_API_KEY"] = key_claude
...
def query_with_claude(context, user_query):
    ...
    message = client.messages.create(
        ...
        max_tokens = 2048,
        temperature = 0,
        system = "You are a helpful assistant specialized in analyzing systems and work systems. You are given a knowledge graph in Turtle format, with a few text annotations, that you will have to analyze every time. Each time, you will base your answer solely on the given knowledge graph extract without external information.",
        messages = [
            {
                "role": "user",
                "content": [
...
def main():
    ...
    final_response_claude = query_with_claude(context, prompt)

    print("FINAL RESPONSE GPT: \n")
    print(final_response_gpt)
    print("FINAL RESPONSE Claude: \n")
    print(final_response_claude)
